...
drawpathnsteps(N,K,X,Y) :- path(X,Y,L),nl,drawmazensteps(N,K,L). % predicatul pt. cerinta (i)/Exercitiul 1/Tema 2

drawpathnstepsfile(N,K,X,Y) :- tell('d:\\temporar\\mazenstepsfile.txt'),path(X,Y,L),nl,drawmazensteps(N,K,L),told.
% acesta e primul predicat pentru cerinta (iii)/Exercitiul 1/Tema 2
...
drawpathnallsteps(N,K,X,Y) :- path(X,Y,L),nl,drawmaze(N,K),drawmazenallsteps(N,K,[],L). % predicatul pt. cerinta (ii)/Exercitiul 1/Tema 2

drawpathnallstepsfile(N,K,X,Y) :- tell('d:\\temporar\\mazenallstepsfile.txt'),path(X,Y,L),nl,drawmaze(N,K),drawmazenallsteps(N,K,[],L),told.
% acesta e al doilea predicat pentru cerinta (iii)/Exercitiul 1/Tema 2

drawmazenallsteps(_,_,_,[]) :- nl.
drawmazenallsteps(N,K,PrefixDrum,[X|T]) :- concat(PrefixDrum,[X],PrefixNou), drawmazestarncirc(N,K,PrefixDrum,X), drawmazenallsteps(N,K,PrefixNou,T).

drawmazestarncirc(0,_,_,_) :- nl.
drawmazestarncirc(N,K,P,X) :- N>0, M is N-1, drawmazestarncirc(M,K,P,X), nl, mazelinestarncirc(N,K,P,X). 

mazelinestarncirc(_,0,_,_).
mazelinestarncirc(N,K,P,(R,C)) :- K>0, G is K-1, mazelinestarncirc(N,G,P,(R,C)), H is K+1,
                                  (N=R,K=C,write('*') ; member((N,K),P),write('o') ; M is N+1,undercell(N,K,M)), rightcell(N,K,H).

/* Dati interogarile:
?- drawpathnsteps(4,6,(1,1),(4,6)).
?- drawpathnstepsfile(4,6,(1,1),(4,6)).
?- drawpathnallsteps(4,6,(1,1),(3,5)).
?- drawpathnallstepsfile(4,6,(1,1),(3,5)).
*/
